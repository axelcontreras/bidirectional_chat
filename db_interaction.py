    ...
    with (connect_to_db()) as conn:
        with (conn.cursor()) as cursor:
            try:
                ...
                raise("Error mientras insertaba datos de usuario:", error)
                
# Obtiene la informacion de un usuario por su username
def get_user_by_id(username):
    with (connect_to_db()) as conn:
        with (conn.cursor()) as cursor:
            try:
                cursor.execute("SELECT user_id FROM users WHERE username = %s", (username,)) # Se define con , al final para que sea una tupla
                user = cursor.fetchone()
                return user[0]
            except (Exception, psycopg2.Error) as error:
